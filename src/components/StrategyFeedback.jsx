...
function FrequencyBar({ action, ev, isChosen, isBest }) {
  const pct = simplifyFrequency(action.frequency);
  return (
    <div className={`flex items-center gap-3 py-2 px-3 rounded-lg ${isChosen ? 'bg-white/5 ring-1 ring-white/10' : ''}`}>
      ...
      <span className="w-10 text-right text-sm font-mono text-gray-400">{pct}%</span>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          onClick={onNext}
          className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-5 py-2.5 rounded-xl font-semibold text-sm transition-transform duration-100 ease-out hover:-translate-y-0.5 active:scale-95 cursor-pointer"
        >
          {isLastHand ? 'Finish Drill' : 'Next Hand'}
          {isLastHand ? <Trophy size={16} /> : <ArrowRight size={16} />}
        </button>
      </div>
